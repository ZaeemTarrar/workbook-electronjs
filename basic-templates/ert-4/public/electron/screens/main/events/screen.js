...
module.exports = (id) => {
	...
	Screen.once('ready-to-show', () => {
		if (consoleEvents)
			console.log('Main Window - Visible: ', Screen.getBounds(), Screen.getContentBounds(), Screen.getPosition());
		Screen.show();
	...
	Screen.on('always-on-top-changed', (event, isAlwaysOnTop) => {
		if (consoleEvents) console.log('Main Window - Always on Top Changed: ', isAlwaysOnTop);
	});
